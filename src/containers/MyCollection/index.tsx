...
const ProfileContainer = () => { 
	  ...
	  const transFromTVP = async () => { 
		try {
			...
			if(Number(fromTokenAmt) > Number(availToken)){
				alert('You do not have enough TVP in your possession.')
				return;
			}
	...
	const transFromTVS = async () => {
		...
		let availToken = new BigNumber($('#availToken>span>span>em').text());
		 ...
		 if(Number(fromTokenAmt) > Number(availToken)){
		 	alert('You do not have enough TVS in your possession.')
		 	return;
		 }
	...
	const chgSwapToken = () =>{
		...
		if($('#availToken>span').eq(1).text().indexOf('TVP') >= 0 ){//TVS -> TVP
			$('#availToken>span').eq(1).html($('#availToken>span').eq(1).html().replace('TVP','TVS'));
			$('#availToken>span>span>em').text(tvsAmount);
			$('#fromToken').text('TVS');
			$('#toToken').text('TVP');
			$('#toTokenQnt').text('TVP Quantity');
			$('#toTokenAmt').val(0);
			$('#fromTokenAmt').val(0);
		...
		}else if($('#availToken>span').eq(1).text().indexOf('TVS') >= 0){//ETH -> TVP
			$('#availToken>span').eq(1).html($('#availToken>span').eq(1).html().replace('TVS','ETH'));
			...
			setFromToken('MATIC');
		}else if($('#availToken>span').eq(1).text().indexOf('MATIC') >= 0){//TVP -> TVS
			$('#availToken>span').eq(1).html($('#availToken>span').eq(1).html().replace('MATIC','TVP'));
			$('#availToken>span>span>em').text(tvpAmount);
			$('#fromToken').text('TVP');
			$('#toToken').text('TVS');
			$('#toTokenQnt').text('TVS Quantity');
			$('#toTokenAmt').val(0);
			$('#fromTokenAmt').val(0);
			$('.exchgPop>.exchgCont>.inputSec>ul>li>p.subTit>span').css('font-size','13px');
			$('#fromTokenAmt').css('width','84%');
	...
	const tokenSwapCall = () =>{
		if($('#availToken>span').eq(1).text().indexOf('TVP') >= 0 ){
			// 메타마스크 연동 없이 컨트롤 가능한 회사지갑에서 바로 송금
			transFromTVP();
		}else if($('#availToken>span').eq(1).text().indexOf('TVS') >= 0 ){
			// 메타마스크 통해 토큰스왑(TVS) 신청
			transFromTVS();
  ...
  return (
	...
	<div className="exchgPop clsePop">
		...
		<div className="inputSec">
			...
			<li>
				<p className="subTit" id='toTokenQnt'>TVS Quantity</p>
				<div className="inputArea">
